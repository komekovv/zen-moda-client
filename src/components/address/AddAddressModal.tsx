...
import { usePostUserLocationMutation } from '@/hooks/useAddress';
import { LocationResponse } from '@/api/queryTypes/Location';
import { MapPin, User, Phone, Map, Plus, AlertCircle } from 'lucide-react';

interface AddAddressModalProps {
  onAddressAdd?: (address: LocationResponse) => void;
}

// Global instance for use with modal context
let globalOnAddressAdd: ((address: LocationResponse) => void) | null = null;

export const setGlobalAddressHandler = (handler: (address: LocationResponse) => void) => {
...
const AddAddressModal: React.FC<AddAddressModalProps> = ({ onAddressAdd }) => {
  ...
  const postLocationMutation = usePostUserLocationMutation();
  ...
  const onSubmit = async (data: AddressFormData) => {
    try {
      ...
      const newAddress = await postLocationMutation.mutateAsync(data);
        ...
        <div className="space-y-6 w-full">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 sm:space-y-5">
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-4">
              <button
                  type="button"
                  onClick={handleClose}
                  disabled={postLocationMutation.isPending}
              ...
              <button
                  type="submit"
                  disabled={!isValid || postLocationMutation.isPending}
                  className={`flex-1 py-2 sm:py-3 px-4 rounded-lg font-medium transition-all duration-200 flex items-center justify-center gap-2 text-body-description-mobile sm:text-body-description ${
                      isValid && !postLocationMutation.isPending
                          ? 'bg-primary hover:bg-primary/90 text-white shadow-md hover:shadow-lg'
                          : 'bg-passive text-white cursor-not-allowed'
                  }`}
              >
                {postLocationMutation.isPending ? (
                    <>
          ...
          {postLocationMutation.isError && (
              <div className="bg-red-50 border border-sale rounded-lg p-3 sm:p-4 flex items-center">
                ...
                <div>
                  <h4 className="text-sale font-medium text-body-description-mobile sm:text-body-description">{t('add_failed')}</h4>
